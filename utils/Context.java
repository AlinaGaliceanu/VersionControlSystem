    ...
    public void run() {
        ...
        while (true) {
            ...
            if (!wasError && this.isTrackable(operation)) {
                ...
                switch (operation.getType().toString().toLowerCase()) {
                    case "touch":
                        vcs.staging.add("Created file " + operation.getOperationArgs().get(1));
                        break;
                    case "mkdir":
                        vcs.staging.add("Created directory " + operation.getOperationArgs().get(1));
                        break;
                    case "rm":
                    case "rmdir":
                        vcs.staging.add("Removed " + operation.getOperationArgs().get(1));
                        break;
                    case "writetofile":
                        vcs.staging.add("Added \"" + operation.getOperationArgs().get(0) +
                                "\" to file " + operation.getOperationArgs().get(1));
                        break;
                    case "cd":
                        vcs.staging.add("Changed directory to " + operation.getOperationArgs().get(1));
                        break;
                    default:
                        break;
